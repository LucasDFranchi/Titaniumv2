...
public:
    ...
    int16_t GetSerializedSize(void) const {
        return (sizeof(this->_ap_status) + sizeof(this->_sta_status) + 2);
    }
    ...
    int16_t Serialize(char* out_buffer, uint16_t out_buffer_size) const {
        ...
        out_buffer[data_position++] = sizeof(this->_ap_status);
        memcpy(&out_buffer[data_position], &this->_ap_status, sizeof(this->_ap_status));
        data_position += sizeof(this->_ap_status);

        out_buffer[data_position++] = sizeof(this->_sta_status);
        memcpy(&out_buffer[data_position], &this->_sta_status, sizeof(this->_sta_status));
        data_position += sizeof(this->_sta_status);
    ...
    int8_t DeSerialize(const char* in_buffer, uint16_t in_buffer_size) {
        ...
        memcpy(&this->_ap_status, &in_buffer[data_position], size);
        data_position += size;
    
        size = in_buffer[data_position++];
        if (size + data_position > in_buffer_size) return 0;
        memcpy(&this->_sta_status, &in_buffer[data_position], size);
        data_position += size;
