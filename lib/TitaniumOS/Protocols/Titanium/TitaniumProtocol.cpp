 ...
 */
esp_err_t TitaniumProtocol::ValidateUUID(uint32_t uuid) {
 ...
 */
esp_err_t TitaniumProtocol::ValidateCRC(uint32_t crc, uint8_t* buffer, uint16_t size) {
 ...
 */
esp_err_t TitaniumProtocol::EncodePayloadLength(uint8_t* buffer, uint16_t payload_length) {
 ...
 * @return `ESP_OK` if encoding is successful, otherwise an error code.
 */
esp_err_t TitaniumProtocol::EncodeAddress(uint8_t* buffer, uint16_t address) {
 ...
 * @return `ESP_OK` if encoding is successful, otherwise an error code.
 */
esp_err_t TitaniumProtocol::EncodeCRC(uint8_t* buffer, uint16_t offset, uint32_t crc) {
    if (buffer == nullptr) {
        return ESP_FAIL;
    }
 ...
 * @return `ESP_OK` if encoding is successful, otherwise an error code.
 */
esp_err_t TitaniumProtocol::EncodeUUID(uint8_t* buffer, uint32_t uuid) {
 ...
 */
esp_err_t TitaniumProtocol::Decode(uint8_t* buffer, size_t size, std::unique_ptr<TitaniumPackage>& package) {
    ...
    do {
        ...
        if (result == ESP_OK) {
            package.reset(
                new TitaniumPackage(payload_length, address, command, memory_area, payload));
        }
    } while (0);
