 ...
 *
 * @return ESP_OK if the key extraction is successful and within size limits, otherwise Error::UNKNOW_FAIL.
 */
titan_err_t GetRequestKey(httpd_req_t* req, const char* buffer, uint16_t* key) {
    ...
    do {
        ...
        if (memcpy_s(array_key,
                     const_cast<char*>(&req->uri[key_string_start]),
 ...
 *
 * @return ESP_OK if data reception is successful and within size limits, otherwise Error::UNKNOW_FAIL.
 */
