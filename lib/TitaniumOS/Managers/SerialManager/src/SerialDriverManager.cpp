...
//  *
//  * This function initializes the SerialDriverManager and continuously reads data
//  * from UART_NUM_0. If the initialization fails, the process handler is deleted.
//  * The function reads data from UART_NUM_0 and writes it to the display area in
//  * memory if the length of the data is 1024 bytes. The function delays for 50
...
//     while (1) {
//         auto len = uart_read_bytes(UART_NUM_0, this->input_buffer_,
//                                    this->buffer_size_, pdMS_TO_TICKS(200));

//         if (len > 0) {
//             auto result = serial_protocol.ProcessIncomingMessage(
//                 this->input_buffer_, len);
...
//     result +=
//         uart_driver_install(UART_NUM_0, this->buffer_size_ * 2, 0, 0, NULL, 0);
