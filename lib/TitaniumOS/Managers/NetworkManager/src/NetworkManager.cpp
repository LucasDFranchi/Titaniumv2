 ...
 *
 * @param[in] arg Pointer to user data (NetworkManager instance).
...
void NetworkManager::Execute(void) {
    ...
    while (1) {
        if (shared_memory_manager->IsAreaDataUpdated(ManagersAreaIndex::CREDENTIALS)) {
            this->SetStationMode(&this->_sta_config);
            esp_wifi_connect();
        }

        if (this->_need_update_network_data) {
            shared_memory_manager->Write(ManagersAreaIndex::CONNECTION,
...
void NetworkManager::SetCredentials(wifi_config_t* wifi_config) {
    this->_shared_memory_manager->Read(ManagersAreaIndex::CREDENTIALS, &this->_cred_area);
