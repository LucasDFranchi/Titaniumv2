...
esp_err_t NetworkManager::Initialize_(void){
    ...
    memcpy_s(wifi_config.sta.ssid, this->cred_area_.raw_data.sta_ssid, sizeof(wifi_config.sta.ssid));  
    memcpy_s(wifi_config.sta.password, this->cred_area_.raw_data.sta_password, sizeof(wifi_config.sta.password));  
...
void NetworkManager::Execute(void){
    ...
    while(1){
        memory_manager->Write(CONNECTION_AREA, sizeof(this->connection_area_.raw_data), this->connection_area_);
...
void NetworkManager::SetWiFiConnection_(uint8_t status){
    this->connection_area_.raw_data.connection_status = status;
}
