...
void test_ReadResponse() {
    ...
    std::unique_ptr<TitaniumPackage> package = std::make_unique<TitaniumPackage>(sizeof(serialized_buffer), address, READ_RESPONSE_SECURE_COMMAND, ProtobufIndex::LORA_RECEIVE, (uint8_t*)serialized_buffer);
    ...
    TEST_ASSERT_EQUAL(READ_RESPONSE_SECURE_COMMAND, package->command());
    ...
    std::unique_ptr<TitaniumPackage> package_rec = NULL;
    TEST_ASSERT_EQUAL(ESP_OK, protocol.Decode(driver->buffer, sizeof(driver->buffer), package_rec));
    TEST_ASSERT_NOT_NULL(package_rec);

    uint32_t rec_uuid = 0;
    auto received_bytes_ack = package_rec.get()->Consume((uint8_t*)&rec_uuid);
    TEST_ASSERT_GREATER_THAN(0, received_bytes_ack);

    /* Compare */
    TEST_ASSERT_EQUAL(ACK_COMMAND, package_rec->command());
    TEST_ASSERT_EQUAL(ProtobufIndex::LORA_RECEIVE, package->memory_area());
    TEST_ASSERT_EQUAL(rec_uuid, package->uuid());
}
